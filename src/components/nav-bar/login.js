...
class Login extends React.Component {
  ...
  handleSubmit (event) {
    ...
    .then(response => {
      this.setState({items: response.data, isLoaded: true, submitDone: true, isLogged: true})
    })
    .catch(error => {
    this.setState({
        isLoaded: true,
  ...
  render() {
    ...
    return (
      <div>
        ...
        {this.state.submitDone && <Navigate to={`/perfil/${items?.user?.id}/${items.token}`} replace={true} />}
      </div>
    );
  }
}
